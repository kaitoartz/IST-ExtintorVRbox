...
Shader "SH_Vefects_VFX_URP_Particles_Erosion_01_Quest"
{
	Properties
	{
		...
		[Toggle(_QUEST3_FAST_MODE)] _Quest3FastMode("Quest 3 Fast Mode", Float) = 0
	...
	SubShader
	{
		...
		#pragma shader_feature_local _QUEST3_FAST_MODE
		// ensure rendering platforms toggle list is visible

		#include "Packages/com.unity.render-pipelines.core/ShaderLibrary/Common.hlsl"
		#include "Packages/com.unity.render-pipelines.core/ShaderLibrary/Filtering.hlsl"

		// Shared sampler declarations for all passes to keep helper utilities available
		...
		float2 CalculateMotionVectorFromClipSpace( float4 clipPos, float4 prevClipPos )
		{
			float wCurr = max( clipPos.w, 1e-5f );
			float wPrev = max( prevClipPos.w, 1e-5f );
			float2 ndcCurr = clipPos.xy / wCurr;
			float2 ndcPrev = prevClipPos.xy / wPrev;
			float2 motion = ndcCurr - ndcPrev;
			motion *= float2( 0.5f, -0.5f );
			return motion;
		}

		half SamplePowSaturate( half value, half exponent )
		{
			value = saturate( value );
			#ifdef _QUEST3_FAST_MODE
				return value;
			#else
				return pow( max( value, 1e-4h ), max( exponent, 0.0001h ) );
			#endif
		}

		void ComputeErosionData( float2 baseUV, float4 customData, half windPhase, out half maskValue, out half opacityValue )
		{
			half distortion = 0.0h;
		#ifndef _QUEST3_FAST_MODE
			half2 distortionUV = baseUV * _NoiseDistortionScale;
			half2 distortionPan = windPhase * _NoiseDistortionSpeed + distortionUV;
			distortion = tex2D( _NoiseDistortionTexture, distortionPan ).r * 0.1h * _Distortion;
		#endif

			half2 maskUV = baseUV * _MaskScale + _MaskOffset;
			half2 maskPan = maskUV + windPhase * half2( _MaskSpeed, 0.0h );
		#ifndef _QUEST3_FAST_MODE
			maskPan += distortion * _DistortionIntensity;
		#endif
			half maskSample = tex2D( _MaskTexture, maskPan ).r;
			maskSample = saturate( SamplePowSaturate( maskSample, _MaskPower ) * _MaskMultiply );

			half2 moveUV = baseUV * _MaskMoveScale + customData.zw;
			half moveSample = tex2D( _MaskMoveTexture, moveUV ).r;
			moveSample = saturate( SamplePowSaturate( moveSample, _MaskMovePower ) * _MaskMoveMultiply );

			half noiseSample = 1.0h;
		#ifndef _QUEST3_FAST_MODE
			half2 noise01UV = baseUV * _Noise01Scale + windPhase * _Noise01Speed;
			half2 noise02UV = baseUV * _Noise02Scale + windPhase * _Noise02Speed;
			half noise01 = tex2D( _Noise_01_Texture, distortion + noise01UV ).r;
			half noise02 = tex2D( _Noise_02_Texture, distortion + noise02UV ).r;
			half noiseCombined = noise01 * noise02;
			noiseSample = saturate( SamplePowSaturate( noiseCombined, _NoisesPower ) * _NoisesMultiply );
		#else
			noiseSample = saturate( _NoisesMultiply );
		#endif

			maskValue = saturate( maskSample * moveSample ) * noiseSample;
			opacityValue = saturate( maskValue * _OpacityBoost ) - ( customData.x + _Dissolve );
		}
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				half windPhase = ( _WindSpeed * _TimeParameters.x );
				half temp_output_89_0;
				half temp_output_97_0;
				ComputeErosionData( IN.ase_texcoord3.xy, IN.ase_texcoord4, windPhase, temp_output_89_0, temp_output_97_0 );
		...
		Pass
		{
			...
			{
				float4 clipPos : SV_POSITION;
				float4 prevClipPos : TEXCOORD0;
				float4 ase_color : COLOR;
				float4 ase_texcoord : TEXCOORD1;
			...
			VertexOutput vert ( VertexInput v )
			{
				...
				// URP's helper struct does not expose previous clip space on all backends, so fall back to current pos when unavailable.
				o.prevClipPos = vertexInput.positionCS;
				o.ase_color = v.ase_color;
				o.ase_texcoord = v.ase_texcoord;
				o.ase_texcoord1 = v.ase_texcoord1;
				return o;
			...
			half4 frag ( VertexOutput IN ) : SV_Target
			{
				...
				half maskValue;
				half opacityValue;
				ComputeErosionData( IN.ase_texcoord.xy, IN.ase_texcoord1, windPhase, maskValue, opacityValue );
				half alpha = saturate( IN.ase_color.a * opacityValue );

				float2 motion = CalculateMotionVectorFromClipSpace( IN.clipPos, IN.prevClipPos );
				motion *= alpha;

				return half4( motion, 0.0h, 0.0h );
			}
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				half temp_output_89_0;
				half temp_output_97_0;
				ComputeErosionData( IN.ase_texcoord2.xy, IN.ase_texcoord3, windPhase, temp_output_89_0, temp_output_97_0 );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				half temp_output_89_0;
				half temp_output_97_0;
				ComputeErosionData( IN.ase_texcoord.xy, IN.ase_texcoord1, windPhase, temp_output_89_0, temp_output_97_0 );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				half temp_output_89_0;
				half temp_output_97_0;
				ComputeErosionData( IN.ase_texcoord1.xy, IN.ase_texcoord2, windPhase, temp_output_89_0, temp_output_97_0 );
...
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;1;0,0;Float;False;True;-1;2;UnityEditor.ShaderGraphUnlitGUI;0;13;SH_Vefects_VFX_URP_Particles_Erosion_01_Quest;2992e84f91cbeb14eab234972e07ea9d;True;Forward;0;1;Forward;8;False;False;False;False;False;False;False;False;False;False;False;False;True;0;False;;True;True;0;True;_Cull1;False;False;False;False;False;False;False;False;False;True;False;0;False;;255;False;;255;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;False;False;False;False;True;4;RenderPipeline=UniversalPipeline;RenderType=Transparent=RenderType;Queue=Transparent=Queue=0;UniversalMaterialType=Unlit;True;3;True;12;all;0;True;True;2;5;True;_Src1;10;True;_Dst1;1;1;False;;10;False;;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;True;True;True;True;0;False;;False;False;False;False;False;False;False;True;False;0;False;;255;False;;255;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;True;True;2;True;_ZWrite1;True;3;True;_ZTest1;True;True;0;False;;0;False;;True;1;LightMode=UniversalForwardOnly;False;False;0;;0;0;Standard;23;Surface;1;638311092170185038;  Blend;0;0;Two Sided;1;0;Forward Only;0;0;Cast Shadows;1;0;  Use Shadow Threshold;0;0;Receive Shadows;1;0;GPU Instancing;1;0;LOD CrossFade;0;0;Built-in Fog;0;0;DOTS Instancing;0;0;Meta Pass;0;0;Extra Pre Pass;0;0;Tessellation;0;0;  Phong;0;0;  Strength;0.5,False,;0;  Type;0;0;  Tess;16,False,;0;  Min;10,False,;0;  Max;25,False,;0;  Edge Length;16,False,;0;  Max Displacement;25,False,;0;Vertex Position,InvertActionOnDeselection;1;0;0;10;False;True;True;True;False;False;True;True;True;True;False;;False;0
